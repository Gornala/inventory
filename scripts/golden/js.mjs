...
const numbers = [
  0, 1, -1, 0.1, 0.2, 0.1 + 0.2, 4.7, 4.700000000000001, 5.62e3, 100, 1e21, 1e-7, 1.5e-7,
  0.000001, 0.0000015, 123456789012345680000, 2.5, 1.005, 0.125, 1.45, -2.5, -0.5, 0.5,
  1234565, 4.35, 22e-12, 100e-9, 150e-6, 5e-3, 1e6, 999999.5, 0.49999999999999994,
];
for (let i = 0; i < 1500; i++) {
  const magnitude = 10 ** Math.floor(rand() * 34 - 12);
  const sign = rand() < 0.15 ? -1 : 1;
  const x = rand() < 0.3 ? Math.round(rand() * 2000) / pick([1, 2, 4, 8, 10, 100, 1000]) : rand() * magnitude;
  numbers.push(sign * x);
}

const numStr = numbers.map((x) => [x, String(x)]);
const toFixed = [];
const toPrecision = [];
const round = [];
...
const words = [
  "R|10k|0402", "R|1k|0402", "C|100n|0603", "C|10u|0805", "digikey", "DigiKey", "lcsc", "mouser",
  "R2", "R10", "R100", "0402", "0603", "1206", "a-b", "a_b", "x.y", "x-y", "U|RP2040|QFN-56",
  "FB|600|0603", "Resistor_SMD", "resistor_smd", "10.0", "9.0", "10.0.1", "", "a", "A", "aa", "Aa",
];
for (let i = 0; i < 1500; i++) {
  let w = "";
